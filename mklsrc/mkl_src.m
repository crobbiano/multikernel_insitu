...
%     runNum = 1;
    ...
    %% BASELINE - Loop to get all sparse coeffs
    do_baseline = 0;
    if do_baseline
        t = 0;
    ...
    results.htrain = htrain;
    results.htest = htest;
    results.hvalid = hvalid;
    results.trainClassSmall = trainClassSmall;
    results.testClassSmall = testClassSmall;
    results.validClassSmall = validClassSmall;
    results.classes = classes;
    
    save(['results/results_batch' num2str(results.batchSize) '_run' num2str(runNum) '_conf.mat'], 'results');
    
    %% Plot?
    cm_valid_all = zeros(31,31,25);
    for ii=1:25
        I=eye(31);
        load(['C:\Users\iamchris\Documents\multikernel_journal\mklsrc\results\results_batch50_run' num2str(ii) '_new.mat'])
        figure(1)
        [~, cm_base, ~, ~] = confusion(num2bin10(results.trainClassSmall, length(results.classes)), num2bin10(results.htrain, length(results.classes)));
        ...
        figure(2)
        [~, cm_test, ~, ~] = confusion(num2bin10(results.testClassSmall, length(results.classes)), num2bin10(results.htest, length(results.classes)));
        ...
        figure(3)
        [~, cm_valid, ~, ~] = confusion(num2bin10(results.validClassSmall, length(results.classes)), num2bin10(results.hvalid, length(results.classes)));
        imagesc(normc(cm_valid));
        % colormap((gray))
        colorbar
        caxis([0 1])
        % title('Confusion matrix for generalization data')
        m(ii) = trace(I*cm_valid)/(norm(I, 'fro')* norm(cm_valid,'fro'))
        
        cm_valid_all(:,:,ii) = cm_valid;
    end
    cm_valid_avg = mean(cm_valid,3);
    hr = prod(diag(cm_valid_avg))/det(cm_valid_avg)
end